...
/*---------------------------------------------------------------------------/
/ Eos / original Xbox (RXDK) local addition -- NOT part of upstream FatFs.
/ Tells ff.h that BYTE/WORD/DWORD/UINT/WCHAR already exist (from xtl.h) and it
/ should only add QWORD, instead of retypedef'ing all of them and colliding
/ with xtl.h's own definitions. See the matching `#elif defined
/ (EOS_XBOX_INTTYPES)` branch in ff.h -- that's the only other place this
/ macro is referenced.
/---------------------------------------------------------------------------*/
#define EOS_XBOX_INTTYPES 1

/*---------------------------------------------------------------------------/
/ Another local addition. ff.h's FATFS.win[] buffer is declared with
/ __attribute__((aligned(FS_FATFS_WINDOW_ALIGNMENT))), but that macro is never
/ defined anywhere in ff.h/ffconf.h/diskio.h -- upstream Zephyr supplies it
/ externally via their own CMake build. Vendoring these files directly (no
/ Zephyr build system) needs a fallback or this doesn't compile at all. We
/ have no DMA/cache-line alignment requirement (LPC port-by-port access, not
/ hardware DMA into this buffer), so a small, safe default is all this needs.
/---------------------------------------------------------------------------*/
